...
from . import forms, ScheduleChant, ScheduleArchi, phaseApp, chantierApp, responsableApp
...
def scheduleArchi(request):
    return ScheduleArchi.schedule(request)

def refreshScheduleAchat(request):
    return ScheduleArchi.refreshSchedule(request)
...
def responsableNew(request):
    return responsableApp.New(request)

def responsableModif(request, resp_id):
    return responsableApp.Modif(request, resp_id)